...
public class XBee implements IXBee {
	...
	private final static Logger log = LoggerFactory.getLogger(XBee.class);
	...
	private void doStartupChecks() throws XBeeException {
		...
		try {
			...
			log.debug("Sending AP command for doStartupChecks");
			//AtCommandResponse ap = this.sendAtCommand(new AtCommand("AP"));
			AtCommandResponse ap = (AtCommandResponse)this.sendSynchronous(new AtCommand("AP"));
			...
			if (ap.getValue()[0] != 2) {
				...
				AtCommandResponse ap2 = (AtCommandResponse)this.sendSynchronous(new AtCommand("AP",2));
			...
			log.debug("Sending HV command for doStartupChecks");
			//ap = this.sendAtCommand(new AtCommand("HV"));
			AtCommandResponse ap3 = (AtCommandResponse)this.sendSynchronous(new AtCommand("HV"));
			...
			log.debug("Sending VR command for doStartupChecks");
			//AtCommandResponse vr = this.sendAtCommand(new AtCommand("VR"));
			AtCommandResponse vr = (AtCommandResponse)this.sendSynchronous(new AtCommand("VR"));
	...
	public void sendPacket(int[] packet)  throws IOException {
		...
		xbeeConnection.write(ByteUtils.toBase16(packet));
			...
			pl = new PacketListener() {
				...
				public void processResponse(XBeeResponse response) {
					...
					if (response instanceof XBeeFrameIdResponse){
						...
						log.debug("response IS NOT instanceof XBeeFrameIdResponse");
					}
	...
	public boolean isConnected() {
		...
		try {
			...
			log.info("parser.getXBeeConnection().isConnected()? ");	
			
			if(parser != null)
			{
				if (parser.getXBeeConnection().isConnected()) {			
					return true;
				}
			}
			...
			log.error("isConnected exception=",e);
